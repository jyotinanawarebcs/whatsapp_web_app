        ...
        <Tabs value={activeAdminTab} onValueChange={setActiveAdminTab} className="space-y-6">
          ...
          <TabsContent value="overview" className="space-y-6">
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              <StatCard title="Total Users" value={adminStats?.totalUsers ?? "0"} icon={<Users className="h-5 w-5" />} />
              <StatCard
                title="Credits Distributed"
                value={adminStats?.totalCreditsAllocated?.toLocaleString?.() ?? "0"}
                icon={<Coins className="h-5 w-5" />}
              />
              <StatCard
                title="Active Campaigns"
                value={adminStats?.campaignStats?.activeCampaigns?.toLocaleString?.() ?? "0"}
                icon={<Activity className="h-5 w-5" />}
              />
              <StatCard
                title="Scheduled Campaigns"
                value={adminStats?.campaignStats?.scheduledCampaigns?.toLocaleString?.() ?? "0"}
                ...
                <CardContent className="space-y-3">
                  <div className="rounded-lg border p-4">
                    ...
                    <p className="mt-2 text-2xl font-semibold">
                      {adminStats?.campaignStats?.totalCampaigns?.toLocaleString?.() ?? "0"}
                    </p>
                  </div>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="rounded-lg border p-4">
                      ...
                      <p className="mt-2 text-xl font-semibold">
                        {adminStats?.campaignStats?.activeCampaigns?.toLocaleString?.() ?? "0"}
                      </p>
                    </div>
                    <div className="rounded-lg border p-4">
                      <p className="text-sm text-muted-foreground">Scheduled</p>
                      <p className="mt-2 text-xl font-semibold">
                        {adminStats?.campaignStats?.scheduledCampaigns?.toLocaleString?.() ?? "0"}
                      </p>
                    </div>
                  </div>
                </CardContent>
                ...
                <CardContent className="space-y-3">
                  <div className="rounded-lg border p-4">
                    ...
                    <p className="mt-2 text-3xl font-semibold">
                      {adminStats?.totalCreditsAllocated?.toLocaleString?.() ?? "0"}
                    </p>
                  </div>
                  <p className="text-sm text-muted-foreground">
          ...
          <TabsContent value="numbers" className="space-y-6">
            {adminStats?.numberHealth ? (
              <>
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                  <StatCard
                    title="Virtual Numbers"
                    value={adminStats?.numberHealth?.totalVirtualNumbers ?? "0"}
                ...
                <div className="grid gap-6 lg:grid-cols-2">
                  <Card>
                    ...
                    <CardContent className="space-y-3">
                      {Object.entries(adminStats?.numberHealth?.statusBreakdown ?? {}).map(
                        ([status, count]) => (
                          <div
                            key={status}
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-3">
                      {Object.entries(adminStats?.numberHealth?.qualityBreakdown ?? {}).map(
                        ([quality, count]) => (
                          <div
                            key={quality}
                            className="flex items-center justify-between rounded-lg border p-3"
              ...
              <div className="text-center text-sm text-muted-foreground">
                Number health data unavailable.
              </div>
            )}
          </TabsContent>
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  ...
                  <div className="flex items-end">
                    <Button
                      variant="outline"
                      className="w-full"
                      onClick={() => handleLogFilterChange({ source: "", eventType: "" })}
                    >
                      Clear Filters
                    </Button>
                  </div>
                      ...
                      {logs.map((log) => (
                        <div key={log.id} className="grid gap-2 p-4 text-sm">
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {log.referenceId && <Badge variant="outline">Ref: {log.referenceId}</Badge>}
                          </div>
                          {log.metadata && (
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {campaigns.map((campaign) => (
              <CampaignCard
                ...
                date={campaign.date}
                icon={
                  iconMap[campaign.icon as keyof typeof iconMap] || (
